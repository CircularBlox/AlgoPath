...
### Changed
- **Landing page redesign — dark code-editor theme**: Full rebuild of `/` into a Tokyo Night / Catppuccin-family editor color scheme, governed by one strict, syntax-theme-disciplined design system applied to every section. Self-contained dark theme scoped to `.landing-root` (renders dark regardless of the global light/dark toggle, no global theme changes) and Geist + JetBrains Mono type pairing (Geist for the 3 display sizes; mono for all labels/code/numbers/metadata). Palette as a syntax theme — each hue has one job: near-black bg `#0B0B10`, one violet-tinted dark surface `#14141C` for **all** cards (hero demo, how-it-works, AI review, capabilities, pricing — nothing inverts to white), violet-tinted 1px borders; **violet** `#A78BFA` brand/eyebrows/headings/Pro, **cyan** `#7DD3FC` numbers/ratings/code identifiers, **mint** `#4ADE80` success, **amber** `#FBBF24` caution, **rose** `#FB7185` failure; no gradient text anywhere. Editor texture: traffic-light + filename-tab + language-pill chrome on the demo and code cards, hand-rolled role-colored syntax highlighting on the Python sample, a faint violet dotted minimap behind the hero, and violet hairline section dividers. Contrast fixed so hero/CTA emphasis and every card heading read at full strength at rest. 8px spacing rhythm, one `rounded-xl`/`p-6` card recipe. Varied structures: split hero with a live hint-panel preview, an algorithm-topic ticker marquee, a four-up stat band, three-step "how it works", IDE-style AI review, an asymmetric bento, pricing cards (Pro differentiated by a violet border + top-rule, not inversion), and a closing CTA. Pure-CSS interactions (staggered reveals, marquee, sparkline draw-in, hover lifts) with full `prefers-reduced-motion` support. (`src/app/page.tsx`, `src/app/globals.css`)
