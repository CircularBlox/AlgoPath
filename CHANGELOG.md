...
### Changed
- **Landing page redesign**: Full rebuild of `/` into a dark, high-end, terminal-native design for competitive programmers. Self-contained dark theme scoped to `.landing-root` (renders dark regardless of the global light/dark toggle, no global theme changes) and Geist + JetBrains Mono type pairing. Richer-but-disciplined color: blue primary dominant, an analogous cyan→violet "tech" gradient family for headline accents and progression, and strictly-semantic emerald/amber/red. Varied section structures: split hero with a live hint-panel preview, an algorithm-topic ticker marquee, a four-up stat band, a three-step "how it works", IDE-style AI code review, an asymmetric bento (problem feed, animated rating sparkline, sample-runner, streak heatmap, multi-platform), gradient-bordered pricing cards, and a closing CTA. Removed the dot-grid/glow-blob hero and the duplicate emoji rank grids. Pure-CSS interactions throughout (staggered reveals, marquee, sparkline draw-in, hover lifts) with full `prefers-reduced-motion` support. (`src/app/page.tsx`, `src/app/globals.css`)
