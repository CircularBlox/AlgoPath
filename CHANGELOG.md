...
### Changed
- **Landing page redesign**: Full rebuild of `/` into a dark, high-end, terminal-native design for competitive programmers, governed by one strict design system applied to every section. Self-contained dark theme scoped to `.landing-root` (renders dark regardless of the global light/dark toggle, no global theme changes) and Geist + JetBrains Mono type pairing. **One** teal-tinted neutral scale, **one** cyan/teal accent, and amber used **only** for caution — no gradient text, no purple/emerald/blue decorative color. One card surface everywhere (dark bg + 1px border + `rounded-xl` + `p-6`), 8px spacing rhythm, and exactly three display sizes. Varied section structures: split hero with a live hint-panel preview, an algorithm-topic ticker marquee, a four-up stat band, a three-step "how it works", IDE-style AI code review, an asymmetric bento (problem feed, animated rating sparkline, sample-runner, streak heatmap, multi-platform), pricing cards (Pro differentiated by a cyan border, not inversion), and a closing CTA. Removed the dot-grid/glow-blob hero and the duplicate emoji rank grids. Pure-CSS interactions throughout (staggered reveals, marquee, sparkline draw-in, hover lifts) with full `prefers-reduced-motion` support. (`src/app/page.tsx`, `src/app/globals.css`)
