...
### Changed
- **Landing page redesign**: Full rebuild of `/` into a dark, terminal-native design for competitive programmers. Self-contained dark theme scoped to `.landing-root` (renders dark regardless of the global light/dark toggle, no global theme changes), Geist + JetBrains Mono type pairing, true-near-black palette with a cold-blue cast, disciplined blue accent plus semantic cyan/green/amber. New sections: split hero with a live hint-panel preview, three-step "how it works", IDE-style AI code review, man-page feature list, terminal-style problem feed, flat pricing cards, closing CTA. Removed the dot-grid/glow-blob hero and the duplicate emoji rank grids. Pure-CSS staggered entrance animations with `prefers-reduced-motion` support. (`src/app/page.tsx`, `src/app/globals.css`)
